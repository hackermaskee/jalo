...
> ⚠️ **Incubation (v0.6.1)** — jalo は活発に開発中です。
> 個人実験プロジェクトゆえ `1.0.0` への到達は永遠にない可能性が高く、
> `0.x.y` 系列での長期運用を前提としています
> (バージョニング方針は SPEC.md §1.2 を参照)。
...
| 数値リテラル | 整数既定 + サフィックス | `42`/`42i`/`42l`/`42d`/`3.14` | 整数は既定で int、`d` は明示 double |
...
- **数値型サフィックス** (`42i`、`42l`): サフィックスなし整数は `int` として解釈され、`i`/`l`/`d` で型を明示できます。
