...
class PatternMatcherTest {
    ...
    void mB1_literalExactMatch() {
        assertThat(evaluator.eval(parse("(match (quote \"x\") (quasiquote \"x\") (quote \"matched\") (quasiquote \"y\") (quote \"other\"))")))
            .isEqualTo(new JaloString("matched"));
    }

    @Test
    void mB2_literalNoMatchReturnsNull() {
        assertThat(evaluator.eval(parse("(match 0i (quasiquote 42i) (quote \"matched\"))"))).isEqualTo(JaloNull.INSTANCE);
    }

    @Test
    void mB3_dollarVariableBind() {
        assertThat(evaluator.eval(parse("(match 5i (quasiquote (var x)) x)"))).isEqualTo(new JaloInt(5));
    }

    @Test
    void mB4_arrayExactLengthMultiBinding() {
        assertThat(evaluator.eval(parse("(match (quasiquote (array 1i 2i)) (quasiquote (array (var a) (var b))) (+ a b))")))
    ...
    void mB5_arrayLengthMismatchFallsBack() {
        String input =
            "(match (quasiquote (array 1i 2i 3i)) (quasiquote (array (var a) (var b))) "
                + "(quote \"matched\") (quasiquote (var x)) (quote \"fallback\"))";
    ...
    void mB6_atSpliceRestArrayBinding() {
        assertThat(evaluator.eval(parse("(match (quasiquote (array 1i 2i 3i)) (quasiquote (array (var head) (rest-seq tail))) (quasiquote (array (var head) (var tail))))")))
    ...
    void mB7_mapKeyValueMatch() {
        assertThat(evaluator.eval(parse("(let [m (quasiquote (map (\"name\" \"Alice\")))] (match m (quasiquote (map (\"name\" (var n)))) n))")))
            .isEqualTo(new JaloString("Alice"));
    }

    @Test
    void mB8_percentRestInMap() {
        assertThat(evaluator.eval(parse("(let [m (quasiquote (map (\"a\" 1i) (\"b\" 2i)))] (match m (quasiquote (map (\"a\" (var v)) (rest-map rest))) rest))")))
            .isEqualTo(JaloMap.empty().put("b", new JaloInt(2)));
    }

    @Test
    void mB9_wildcardUnderscore() {
        assertThat(evaluator.eval(parse("(match 42i (quasiquote (var _)) (quote \"any\"))"))).isEqualTo(new JaloString("any"));
    }

    @Test
    void mB10_allPatternsFailReturnsNull() {
        assertThat(evaluator.eval(parse("(match (quote \"hello\") (quasiquote 42i) (quote \"num\") (quasiquote (array (var x))) (quote \"arr\"))")))
