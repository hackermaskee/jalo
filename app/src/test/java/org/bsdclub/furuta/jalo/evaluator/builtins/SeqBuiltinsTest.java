...
class SeqBuiltinsTest {
    ...
    @Test
    void bB21_empty() {
        assertThat(evaluator.eval(parse("(empty? (quasiquote (array)))"))).isEqualTo(JaloBool.TRUE);
        assertThat(evaluator.eval(parse("(empty? (quasiquote (array 1i)))"))).isEqualTo(JaloBool.FALSE);
    }

    @Test
    void bB22_getIn() {
        assertThat(evaluator.eval(parse("(get-in (quasiquote (map (\"a\" (map (\"b\" 42i))))) (quasiquote (array \"a\" \"b\")))")))
            .isEqualTo(new JaloInt(42));
    }

    @Test
    void bB23_assocIn() {
        assertThat(evaluator.eval(parse("(assoc-in (quasiquote (map (\"a\" (map (\"b\" 1i))))) (quasiquote (array \"a\" \"b\")) 9i)")))
            .isEqualTo(JaloMap.empty().put("a", JaloMap.empty().put("b", new JaloInt(9))));
    }

    @Test
    void bB24_updateIn() {
        assertThat(evaluator.eval(parse("(update-in (quasiquote (map (\"a\" (array 1i 2i)))) (quasiquote (array \"a\")) (fn [x] (reverse x)))")))
