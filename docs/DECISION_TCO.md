...
Phase 1 確定 / Phase 2 再評価予定 — Phase 1 (incubation 期) は Option F（TCO なし）で継続。TCO の実装方式は Phase 2 着手時に改めて確定する（下記「推奨」節を参照）。
...
**推奨: Phase 2 で TCO を実装する。Phase 1 (incubation 期) は当面 Option F (TCO なし / recur 強制) で継続する。Phase 2 着手時に改めて Option A/B/C/D/E の中から実装方式を確定する。**
...
- **Q5: ADR のオープン問い** — この ADR はマージ前に最終的な決定を記録すべきか、「Phase 1 確定 / Phase 2 再評価予定」は incubation 中のプロジェクトとして長期状態として許容されるか。
- **Q6: マクロとの相互作用** — 将来のマクロシステム（VISION.md §6）は TCO 戦略に制約を課すか。特に Option D（CPS）はマクロ展開と相互作用する可能性がある。
- **Q7: バージョン 1.0 のスコープ** — Option F（1.0 まで TCO なし）は Lisp 方言としての jalo のポジショニングを考慮して許容されるか。
