...
- bump 種別の判断に迷う場合は、PR レビューで妥当性確認を行う（CONTRIBUTING.md §9）。
- `1.0.0` 到達条件は、以下の三条件をすべて満たすこと:
  - 言語仕様が安定し、後方互換性を維持できる状態であること。
  - 年単位の deprecation 期間を運用できる体制が整っていること。
  - 非互換変更に対する警告機構を実装側で提供できる能力を持つこと。
- 上記三条件は、Phase 1 / Phase 2 のスコープ分割 (実装フェーズ) とは独立した概念である。Phase 2 機能 (バイトコードコンパイラ・TCO・マクロ機構・名前空間等) の実装の有無は `1.0.0` 到達の前提条件ではない。
- 注意: jalo は個人実験プロジェクトであり、`1.0.0` への到達は永遠に発生しない可能性が高い。`0.x.y` 系列の長期運用は設計上想定された運用形態である。
...
| `trampoline` | `(trampoline f & args)` | `f(args...)` を初回呼出し、返り値が fn の間は引数なしで呼び続け、非 fn になったら返す。深い相互/自己末尾再帰のスタック消費を回避する |
...
**対応状況**: 10 件完全対応 (✅) + 4 件部分対応 (🟡 partial: T-003/T-004/T-007/T-008) = 14 件カバー。
