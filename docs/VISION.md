...
- **Phase 1 のマクロシステム。** 衛生的なマクロ機能は長期ロードマップ（§6）に含まれるが、最初の安定版リリースからは明示的に除外する。
- **Phase 1 の名前空間機構。** モジュールレベルの名前空間（I-02）は先送りとする。現バージョンではすべての定義が単一のグローバルスコープを共有する。
- **Phase 1 の末尾呼び出し最適化（TCO）。** TCO 戦略は現在評価中（I-08、DECISION_TCO.md 参照）。最初のリリースではスタックセーフな末尾再帰を保証しない。深い再帰は `reduce` や `map` といった高階反復関数で代替すること。
...
| マクロシステム | なし | 完全（衛生的） | 完全（Clojure 経由） | `define-syntax` | 計画中（Phase 2 以降） |
| 名前空間 | なし | 完全 | 完全 | `define-library` | 計画中（Phase 2 以降） |
...
- **マクロシステム** — Phase 2 以降に衛生的マクロ機能（仮称 `defmacro`）を導入し、DSL とコード生成パターンを可能にする。
...
- **`1.0.0` リリース** — 到達条件は SPEC.md §1.2 に定義する三条件 (仕様安定 / 年単位 deprecation / 非互換警告機構) による。Phase 2 機能の実装は前提条件ではない。
